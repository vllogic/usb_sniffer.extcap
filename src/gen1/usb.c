...
// gen1 (ataradov USB Sniffer) libusb backend.
//
//   EP 0x82 IN:  continuous capture byte stream (raw frames, no block
//                framing); 512-byte packets aggregated into 1 MiB transfers.
//   EP0 vendor:  FX2LP SRAM (0xa0), I2C (0xb0/0xb1), JTAG (0xc0..0xc2) and
//                capture control register (0xd0) requests.
//
// Adapted for the merged build: usb_open() filters the shared 1209:6688
// VID/PID pair by bcdDevice (gen1 = 0x0001) and the transfer loop watches
// the shared SIGINT stop flag instead of looping forever.
...
static bool g_usb_closing = false;   // teardown: callbacks must not resubmit
...
{
  if (error < 0)
    os_error("%s: %s", text, libusb_error_name(error));
}
...
bool usb_open(int vid, int pid, int bcd_accept)
{
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    // The gen1/gen2 capture devices share VID/PID; only accept the release
    // requested by the caller (gen1 = 0x0001).  bcd_accept == 0 = any.
    if (bcd_accept && desc.bcdDevice != bcd_accept)
      continue;
...
{
  if (usb_open(GEN1_VID, GEN1_PID, GEN1_BCD))
    return;

  if (usb_open(GEN1_VID_LEGACY, GEN1_PID_LEGACY, 0))
    return;

  os_error("could not open a capture device (gen1 1209:6688 rev 0x0001 or legacy 6666:6620)");
}
...
void usb_close(void)
{
  if (g_usb_handle)
  {
    g_usb_closing = true;   // callbacks must not resubmit during teardown
    ...
    // otherwise libusb_exit() below races the callback thread.
...
static void LIBUSB_CALL usb_capture_callback(struct libusb_transfer *transfer)
{
  ...
  // Teardown: usb_close() set g_usb_closing before cancelling the pool.
  // Drop everything at the top: a transfer that completed before the cancel
  // still delivers its buffer during the drain loop, and the capture state
  // may already be gone -- feeding it is a use-after-free.
  if (g_usb_closing)
    return;
  ...
  if (LIBUSB_TRANSFER_TIMED_OUT == transfer->status)
    ...
    return;   // shutdown: usb_close() cancelled the pool, do not resubmit
  else if (LIBUSB_TRANSFER_NO_DEVICE == transfer->status)
  {
...
void usb_data_transfer(void)
{
  ...
  // Drive the event loop until the merged SIGINT handler sets g_stop.
  while (!g_stop)
  {
