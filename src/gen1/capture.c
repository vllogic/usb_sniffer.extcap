...
// gen1 capture engine: pcapng writer + frame parsing + capture init.
// Ported unmodified from the upstream usb-sniffer software/capture.c
// (minus the extcap request handler, which the merged extcap.c provides).
//
// The gen2 packet.c is a structured port of this file's parsing logic; both
// generations consume the same 4-byte status / 7-byte data frame format.
